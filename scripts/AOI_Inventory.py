                    ...
                    for comment in param.find("Comments"):
                        ...
                        for loc_comm in comment.findall("LocalizedComment"):
                            ...
                            if lan in ["en-GB", "sv-SE"]:
                                ...
                                res["xx-XX"] = lan + "-" + text
                        if res["en-GB"].startswith(("F_","UF_", "GF_", "SF_", "AF_")):
                            res["Type"] = "Fault"
                        elif res["en-GB"].startswith(("W_","UW_", "GW_")):
                            res["Type"] = "Warning"
                        elif res["en-GB"].startswith(("M_","UM_", "GM_")):
                            res["Type"] = "Message"
                        else:
                            res["Type"] = "Issue"
                        # Filter short texts: UF_01, W_02, etc.
                        if len(res["en-GB"])>5 and "reserved" not in str.lower(res["en-GB"]):
                            results.append(res)
            # Instance Alarms
                    ...
                    if len(operand)>len(".iDiagnosticX."):
                        ...
                        if param_name in DIAGNOSTIC_WORDS:
                            ...
                            if res["en-GB"].startswith(("F_","UF_", "GF_", "SF_", "AF_")):
                                ...
                                res["Type"] = "Issue"
                                #print("Message type not detected:", res["en-GB"])
                            # Filter short texts: UF_01, W_02, etc.
                            if len(res["en-GB"])>5 and "reserved" not in str.lower(res["en-GB"]):
                                results.append(res)
...
def Alarms_analyze():
    ...
    unique_values = sorted(df["en-GB"].dropna().unique())
    pd.DataFrame({"en-GB": unique_values}).to_csv('Alarm_list_Unique_Values.csv', index=False)

    df_grouped = df.groupby(["PLC", "Area", "Tag", "DaraType"]).size().reset_index(name="Count")
    df_sorted = df_grouped.sort_values(by="Count", ascending=False) 
    df_sorted.to_csv("Alarm_list_Totals.csv", index=False)

    df_grouped = df.groupby(["PLC", "Area", "Tag", "DaraType", "Type", "User_defined"]).size().reset_index(name="Count")
    df_sorted = df_grouped.sort_values(by="Count", ascending=False) 
    df_sorted.to_csv("Alarm_list_Type_Instaces.csv", index=False)

    df = df_grouped.drop(columns=["PLC", "Area", "Tag"])
    df_max = df.loc[df.groupby(["DaraType", "Type", "User_defined"])["Count"].idxmax().values]
    df_max = df_max.reset_index(drop=True)
    df_max.to_csv('Alarm_list_Type_Totals.csv', index=False)
